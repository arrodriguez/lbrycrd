...
UniValue getclaimsintrie(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Arguments:\n"
            "1. \"blockhash\"     (string, optional) the hash of the block\n"
            "                                           which is the active\n"
            "                                           in the chain. If\n"
            "                                           none is given, \n"
            "                                           the latest block\n"
...
UniValue supportToJSON(std::vector<CSupportValue>::const_iterator itSupport)
{
    UniValue ret(UniValue::VOBJ);
    ret.push_back(Pair("txid", itSupport->outPoint.hash.GetHex()));
    ret.push_back(Pair("n", (int)itSupport->outPoint.n));
    ret.push_back(Pair("nHeight", itSupport->nHeight));
    ret.push_back(Pair("nValidAtHeight", itSupport->nValidAtHeight));
    ret.push_back(Pair("nAmount", itSupport->nAmount));
    return ret;
}
