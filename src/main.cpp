...
bool DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, CClaimTrieCache& trieCache, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        for (unsigned int i = 0; i < tx.vout.size(); ++i)
        {
            ...
            CClaimScriptUndoAddOp undoAdd(COutPoint(hash, i), pindex->nHeight);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); ++i)
            {
                ...
                int nValidAtHeight;
                if (SpendClaim(trieCache, coins->vout[txin.prevout.n].scriptPubKey, COutPoint(txin.prevout.hash, txin.prevout.n), coins->nHeight, nValidAtHeight, spentClaims)) {
